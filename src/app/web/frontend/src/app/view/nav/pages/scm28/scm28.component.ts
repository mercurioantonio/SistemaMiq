...
@Component({
  selector: 'app-scm28',
  templateUrl: './scm28.component.html',
  styleUrls: ['./scm28.component.less']
})
export class Scm28Component {
  ...
  ngOnInit(): void {
    this.scambio.selectedMiq$.subscribe((test: any) => {
      ...
      console.log("scm28.component.ts chiama getAnagrafica di: ", this.selectedMiq)
      ...
      console.log("scm28.component.ts riceve l'anagrafica di: ",this.selectedMiq, "con status: ",this.status, this.dataframeData)
...
//      cid: [[], [Validators.required,Validators.pattern("^[0-9]*$"),Validators.minLength(8), Validators.maxLength(8)]],
      ...
      val_calib_B: [[], []],
      calibrazione_C: [[], [Validators.required]],
      val_calib_C: [[], []],  
      calibrazione_D: [[], []],
      val_calib_D: [[], []],   
      val_calib_master_A: [[], []], 
      ...
      y1_strum_C: [[], [Validators.required]],
      y2_strum_C: [[], [Validators.required]],
      y3_strum_C: [[], [Validators.required]],
      y1_master_C: [[], [Validators.required]],
      y2_master_C: [[], [Validators.required]],
      y3_master_C: [[], [Validators.required]],
  ...
  submitForm(): void {
    ...
    console.log("this.pt_int_D: ", this.pt_int_D)
  ...
  calcolaDifferenze() {
    const LSC_x = Number(this.y_rif)+0.15*(Number(this.T_pre.replace(',','.'))/Math.sqrt(3));
    const LIC_x = Number(this.y_rif)-0.15*(Number(this.T_pre.replace(',','.'))/Math.sqrt(3));

    const LSC_r = 2.575*1.693*0.05*Number(this.T_pre.replace(',','.'))
    const LIC_r = 0*1.693*0.05*Number(this.T_pre.replace(',','.'))
    ...
    const differenze_D = [y1_strum_D - y1_master_D, y2_strum_D - y2_master_D, y3_strum_D - y3_master_D];
    ...
    if((y1_strum_D + y2_strum_D + y3_strum_D) / differenze_D.length <= Number(this.pt_int_D.replace(',','.')) - (Number(this.T_pre.replace(',','.'))/2)  || (y1_strum_D + y2_strum_D + y3_strum_D) / differenze_D.length >= Number(this.pt_int_D.replace(',','.'))+ (Number(this.T_pre.replace(',','.'))/2)){
    ...
    const differenze_A = [y1_strum_A - y1_master_A, y2_strum_A - y2_master_A, y3_strum_A - y3_master_A];
    const differenze_B = [y1_strum_B - y1_master_B, y2_strum_B - y2_master_B, y3_strum_B - y3_master_B];
    const differenze_C = [y1_strum_C - y1_master_C, y2_strum_C - y2_master_C, y3_strum_C - y3_master_C];
    ...
    if((y1_strum_A + y2_strum_A + y3_strum_A) / differenze_A.length <= Number(this.pt_int_A.replace(',','.')) - (Number(this.T_pre.replace(',','.'))/2)  || (y1_strum_A + y2_strum_A + y3_strum_A) / differenze_A.length >= Number(this.pt_int_A.replace(',','.'))+ (Number(this.T_pre.replace(',','.'))/2)){
    ...
    if((y1_strum_B + y2_strum_B + y3_strum_B) / differenze_B.length <= Number(this.pt_int_B.replace(',','.')) - (Number(this.T_pre.replace(',','.'))/2)  || (y1_strum_B + y2_strum_B + y3_strum_B) / differenze_B.length >= Number(this.pt_int_B.replace(',','.'))+ (Number(this.T_pre.replace(',','.'))/2)){
    ...
    if((y1_strum_C + y2_strum_C + y3_strum_C) / differenze_C.length <= Number(this.pt_int_C.replace(',','.')) - (Number(this.T_pre.replace(',','.'))/2)  || (y1_strum_C + y2_strum_C + y3_strum_C) / differenze_C.length >= Number(this.pt_int_C.replace(',','.'))+ (Number(this.T_pre.replace(',','.'))/2)){
  ...
  private getWeekNumber(date: Date): number {
   ...
   campione(campione_di_riferimento:string): void{
    ...
    console.log("scm28.component.ts ha individuato questo campione_di riferimento: ", campione_di_riferimento);
