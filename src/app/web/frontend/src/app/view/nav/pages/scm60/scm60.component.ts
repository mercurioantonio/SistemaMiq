...
@Component({
  selector: 'app-scm60',
  templateUrl: './scm60.component.html',
  styleUrls: ['./scm60.component.less']
})
export class Scm60Component {
  ...
  convalidaD: boolean = false;
  convalidaE: boolean = false;
  ...
  ngOnInit(): void {
    this.scambio.selectedMiq$.subscribe((test: any) => {
      ...
      console.log("scm60.component.ts chiama getAnagrafica di: ", this.selectedMiq)
      ...
      console.log("scm60.component.ts riceve l'anagrafica di: ",this.selectedMiq, "con status: ",this.status, this.dataframeData)
      ...
      this.pt_int_D = 'Inserisci il peso'
      this.pt_int_E = 'Inserisci il peso'
...
//      cid: [[], [Validators.required,Validators.pattern("^[0-9]*$"),Validators.minLength(8), Validators.maxLength(8)]],
  ...
  changeD(pt_int_D: any){
    if (pt_int_D != 'Inserisci il peso') {
      this.pt_int_D = pt_int_D;
      this.y1_master_D = pt_int_D;
      this.y2_master_D = pt_int_D;
      this.y3_master_D = pt_int_D;
      this.y4_master_D = pt_int_D;
      this.y5_master_D = pt_int_D;
      this.val_calib_master_D= pt_int_D;
      this.cal_option4 = 'false'
    }
  }

  functionConvalidaD() {
    this.convalidaD = true
  }

  functionModificaD() {
    this.convalidaD = false
  }

  changeE(pt_int_E: any){
    if (pt_int_E != 'Inserisci il peso') {
      this.pt_int_E = pt_int_E;
      this.y1_master_E = pt_int_E;
      this.y2_master_E = pt_int_E;
      this.y3_master_E = pt_int_E;
      this.y4_master_E = pt_int_E;
      this.y5_master_E = pt_int_E;
      this.val_calib_master_E = pt_int_E;
      this.cal_option5 = 'false'
    }
  }

  functionConvalidaE() {
    this.convalidaE = true
  }

  functionModificaE() {
    this.convalidaE = false
  }
    ...
    if (this.validateForm.valid) {
       ...
       console.log(option1, option2, option3, option4, option5)
        ...
        if (check_pt_int_B === 0){
          ...
          else if(option1+option2+option3+option4+option5 === val1+val2+val3+val4+val5){
            if(option1 === 0){
              this.validateForm.value.val_calib_A = null;
              this.validateForm.value.val_calib_master_A = null;
            }
...
calcolaDifferenze_E(){
  ...
  if (this.toll_tecnica_stabilita.includes('%')){
    ...
    var differenze_E = [((y1_strum_E - y1_master_E)/this.pt_int_E.replace(',','.'))*100, ((y2_strum_E - y2_master_E)/this.pt_int_E.replace(',','.'))*100, ((y3_strum_E - y3_master_E)/this.pt_int_E.replace(',','.'))*100, ((y4_strum_E - y4_master_E)/this.pt_int_E.replace(',','.'))*100, ((y5_strum_E - y5_master_E)/this.pt_int_E.replace(',','.'))*100];
    differenze_E = differenze_E.map((valore) => isNaN(valore) || !isFinite(valore) ? 0 : valore);
    var sommaDifferenze_E = differenze_E.reduce((acc, diff) => acc + diff, 0);
  }
  else{
  ...
  showModal(): void {
    ...
    if (Bnull ==1){
      if(this.y1_strum_A == undefined|| this.y2_strum_A == undefined || this.y3_strum_A == undefined || this.y4_strum_A == undefined || this.y5_strum_A == undefined || 
        ...
        if (this.esito_A_x == 'True' && this.esito_A_r == 'True' && this.esito_B_x == 'True' && this.esito_B_r == 'True' && this.esito_C_x == 'True' && this.esito_C_r == 'True' && this.esito_D_x == 'True' && this.esito_D_r == 'True' && this.esito_E_x == 'True' && this.esito_E_r == 'True' && this.validateForm.valid){
          this.submitForm();
          this.showModal_ok();
  ...
  private getWeekNumber(date: Date): number {
   ...
   campione(campione_di_riferimento:string): void{
    ...
    console.log("scm60.component.ts ha individuato questo campione_di riferimento: ", campione_di_riferimento);
