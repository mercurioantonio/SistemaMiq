...
@Component({
  selector: 'app-scm2',
  templateUrl: './scm2.component.html',
  styleUrls: ['./scm2.component.less']
})
export class Scm2Component {
  ...
  predizioni_B: number = NaN;
  predizioni_C: number = NaN;
  predizioni_D: number = NaN;
  out_control: any;
  esito_A_ML: any;
  esito_B_ML: any;
  esito_C_ML: any;
  ...
  ngOnInit(): void {
    this.scambio.selectedMiq$.subscribe((test: any) => {
      ...
      console.log("scm2.component.ts chiama getAnagrafica di: ", this.selectedMiq)
      ...
      console.log("scm2.component.ts riceve l'anagrafica di: ",this.selectedMiq, "con status: ",this.status, this.dataframeData)
...
//      cid: [[], [Validators.required,Validators.pattern("^[0-9]*$"),Validators.minLength(8), Validators.maxLength(8)]],
      ...
      y4_strum_C: [[], [Validators.required]],
      y5_strum_C: [[], [Validators.required]],
      y1_master_C: [[], [Validators.required]],
      y2_master_C: [[], [Validators.required]],
      y3_master_C: [[], [Validators.required]],
      y4_master_C: [[], [Validators.required]],
      y5_master_C: [[], [Validators.required]],
        ...
        if(option1+option2+option3+option4 === 0){
          ...
          this.service.prediction({misure}).subscribe((res: any) => {
            ...
            if (res.statusPrediction == 'success'){
              ...
              this.predizioni_B = Number(res.predizioni_B);
              this.predizioni_C = Number(res.predizioni_C);
              this.predizioni_D = Number(res.predizioni_D);
  ...
  limitML(){
    ...
    if(LIG_x <= this.predizioni_B && this.predizioni_B <= LSG_x ){
      this.esito_B_ML = 'True';
    }
    else{
      this.esito_B_ML = 'False';
    }
    if(LIG_x <= this.predizioni_C && this.predizioni_C <= LSG_x ){
      this.esito_C_ML = 'True';
    }
    else{
      this.esito_C_ML = 'False';
    }
    if(LIG_x <= this.predizioni_D && this.predizioni_D <= LSG_x ){
      this.esito_D_ML = 'True';
    }
    else{
      this.esito_D_ML = 'False';
    }
  ...
  calcolaDifferenze() {
    ...
    if(this.pt_int_D != ''){
      ...
      const differenze_D = [y1_strum_D - y1_master_D, y2_strum_D - y2_master_D, y3_strum_D - y3_master_D, y4_strum_D - y4_master_D, y5_strum_D - y5_master_D];
  ...
  campione(campione_di_riferimento:string): void{
    ...
    console.log("scm2.component.ts ha individuato questo campione_di riferimento: ", campione_di_riferimento);
